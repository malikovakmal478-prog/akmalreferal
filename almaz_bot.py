...
def handle_text(message):
    ...
    if user_id == ADMIN_ID:
        if text == "🛠 Admin Panel":
            ...
            # Raqamni xuddi rasmda ko'rsatilgandek 15,377 formatiga keltirish
            formatted_users = f"{total_users:,}".replace(",", ",")
            
            bot.send_message(user_id, f"📊 Bot statistikasi:\n\n👥 Jami foydalanuvchilar: `{formatted_users}` ta (пользователей)", parse_mode="Markdown", reply_markup=admin_menu())
            return
